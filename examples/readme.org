...
** Using Docker
To enable you to just run the code and get results, we provide a [[./Dockerfile][Dockerfile]]
having Emacs 28.1 (the terminal version =emacs-nox=) and all necessary packages
(especially, Python 3.9.8) installed. It extends the Dockerfile in the parent
directory. To build it run the following commands:
...
- Python 3.9.8
  - with the libraries listed [[../requirements.txt][../requirements.txt]]
- Emacs with the packages
